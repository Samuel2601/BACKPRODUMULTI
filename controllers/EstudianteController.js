	...
	if (req.user) {
		...
		if(id!='null'){
			estudiantes.forEach(element => {
				if(new Date(element.anio_lectivo).getTime()==new Date(id).getTime()){
					pen.push(element);
				}
			});
                   ...
                   bcrypt.hash(data.password,null,null, async function(err,hash){
					...
					if (estudiantes_arr[0].estado == 'Desactivado') {
                        ...
                        //bcrypt.hash(data.password,null,null, async function(err,hash){
                            //if(hash){
                               // data.password = hash;
						let re = await Estudiante.updateOne(
							{ dni: data.dni },
							{
								estado: 'Activo',
								genero: data.genero,
						...
						if (pen.length == 0) {
							...
							await Pension.updateOne(
								{ idestudiante: reg[0].id, anio_lectivo: config.anio_lectivo },
								{
			...
			if (data.length > 0) {
				for (let element of data) {
					...
					if (estudiantes_arr.length == 0 && a.length == 0) {
						...
						var pension = {};
						if (element.dni_factura == undefined && element.dni_padre != undefined) {
							element.dni_factura = element.dni_padre;
							element.nombres_factura = element.nombres_padre;
						}
						if (element.dni_padre == undefined && element.dni_factura != undefined) {
						...
						//  res.status(200).send({message:'Estudiante agregado con exito'});
					} else if ((estudiantes_arr[0]!=undefined && estudiantes_arr[0].estado == 'Desactivado') ||
...
const actualizar_estudiante_admin = async function (req, res) {
	if (req.user) {
		...
		var cn = await Config.find().sort({ createdAt: -1 }); //await Config.findById({_id:'61abe55d2dce63583086f108'});
		let config = cn[0];
		var reg2 = await Pension.find({ idestudiante: id, idanio_lectivo: config._id });
		////console.log(reg2);
		try {
			if (reg2.length == 1) {
				var i = 0;
				...
				if (data.desc_beca == 100) {
					for (let j = 0; j < 10; j++) {
						fecha2.push({
							date: new Date(reg2[i].anio_lectivo).setMonth(new Date(reg2[i].anio_lectivo).getMonth() + j),
						});
					}
					var des = 0;
					fecha2.forEach((element) => {
